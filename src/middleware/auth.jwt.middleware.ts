...
const validToken = function (req: Request, res: Response, next: NextFunction) {
  let token = req.cookies[appConfig.COOKIE_NAME];
      ...
      .then((_decoded) => {
        if (_decoded) {
          ...
          return res
            ...
            .send(appErrorMessage.INVALIDTOKEN);
        }
      })
      .catch((err) => {
        return res
          .status(restApiResponseStatus.InternalServerError)
          .send(appErrorMessage.EXCEPTION);
      });
  } else {
    return res
      .status(restApiResponseStatus.Unauthorized)
